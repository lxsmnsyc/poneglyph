...
import type { Handler } from 'poneglyph/server';
...
async function convertIncomingMessageToRequest(event: H3Event): Promise<Request> {
  const h3 = await import('h3');
...
export function createNodeAdapter(options: ServerOptions) {
  return async function handle(handler: Handler): Promise<void> {
    const h3 = await import('h3');
